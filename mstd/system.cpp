...
bool kill_processes_by_name(const char* process_name) 
{
	...
	while (bProcess) {
		if (0 == _wcsicmp(pe.szExeFile, mstd::to_wstring(process_name).c_str())) {
...
bool get_program_dir(std::string& path) {
    std::string filename;
    if (get_process_path(filename)) {
        std::string::size_type pos = filename.rfind(mstd::system_slash<char>());
...
bool get_program_dir(std::wstring& path) {
    ...
    if (get_process_path(filename)) {
        std::wstring::size_type pos = filename.rfind(mstd::system_slash<wchar_t>());
