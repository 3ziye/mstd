...
#if defined(_USING_STDXX_FILESYSTEM) || defined(_USING_STD_FILESYSTEM) || defined(_USING_BOOST_FILESYSTEM)
#undef _USING_STDXX_FILESYSTEM
#undef _USING_STD_FILESYSTEM
#undef _USING_BOOST_FILESYSTEM
#endif

#define _USING_STDXX_FILESYSTEM 0
#define _USING_STD_FILESYSTEM   0
#define _USING_BOOST_FILESYSTEM 0

#if (defined(STDXX_FILESYSTEM_FORCE) || ( (defined(STD_FILESYSTEM_NOT_SUPPORTED) && !defined(BOOST_FILESYSTEM_AVAILABLE)) && !(defined(BOOST_FILESYSTEM_FORCE) || defined(STD_FILESYSTEM_FORCE)) ) )

#undef  _USING_STDXX_FILESYSTEM 
#define _USING_STDXX_FILESYSTEM  1
...
#undef  _USING_BOOST_FILESYSTEM
#define _USING_BOOST_FILESYSTEM 1
...
#undef _USING_STD_FILESYSTEM 
#define _USING_STD_FILESYSTEM 1
...
namespace stdxx  { namespace experimental { namespace filesystem
{
	...
	public:
        ...
        bool has_parent_path() const { return !parent_path().string().empty(); }
	...
	private:
        //windows ansi, unix utf-8
        void _m_assign(const std::string& source);
	...
	public:
		...
		explicit directory_entry(const ::stdxx::experimental::filesystem::path&);

		//modifiers
		void assign(const ::stdxx::experimental::filesystem::path&);
		void replace_filename(const ::stdxx::experimental::filesystem::path&);
	...
	private:
		::stdxx::experimental::filesystem::path path_;
	};
	...
	{
		return (status(p).type() != file_type::unknown && status(p).type() != file_type::not_found);
	}
...
#       ifdef CXX_NO_INLINE_NAMESPACE
			using namespace stdxx::experimental::filesystem;
#       else
			using namespace stdxx::experimental::filesystem::v1;
#       endif
		} // filesystem
	} // experimental
} // std

#endif	//_USING_STDXX_FILESYSTEM
