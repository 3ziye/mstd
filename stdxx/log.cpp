...
//#endif
        ...
        log_directory_ = stdxx::compose_path(stdxx::get_program_path<char>(), "log");
        prefix_name_ = stdxx::get_last_path(stdxx::get_process_filename<char>());
...
public:
    void set_directory(const char* path) {
        log_directory_ = path;
        stdxx::create_directorys(log_directory_.c_str());
    }
    ...
    void print_v(Level level, const char* filename, unsigned int line,
        ...
        pos += snprintf(buffer + pos, size - pos, "[%04x]", stdxx::get_current_thread_id());
        if (pos >= size) return;

        pos += snprintf(buffer + pos, size - pos, "[%s]", get_level_string(level));
        if (pos >= size) return;

        pos += snprintf(buffer + pos, size - pos, "[%s:%d]", stdxx::get_last_path(filename).c_str(), line);
        if (pos >= size) return;
		...
		auto isCharInString = [](char ch, const char *str) {
		...
		static int key[] = {9,6,8,9,3,1,2,5,7,5,7,9,2,4,6,7,8,3};
		static char ch[] = "[(!@#])$%^&*._+{:}?<>~|\\-=";
		static int char_len = strlen(ch);
		for (size_t i = 0; i < pos; ++i) {
			int shift = key[i % _countof(key)];
			char c = buffer[i];
			if ('0' <= c && c <= '9') {
				c = ((c - '0' + shift) % 10) + '0';
			} else if ('a' <= c && c <= 'z') {
...
private:
    ...
    void clear_files() {
        ...
        for (seconds -= 24 * 3600;; seconds -= 24 * 3600) {
            ...
            if (!stdxx::delete_file(filename.c_str())) break;
        }
    }

private:
    std::string get_log_filename(const char* suffix) {
        return stdxx::compose_path(log_directory_, prefix_name_ + suffix);
    }
